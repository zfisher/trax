...
def linspace(start, stop, num=50, endpoint=True, retstep=False, dtype=float):
  ...
  if endpoint:
    ...
    if num > 1:
      ...
      result = tf.linspace(start.data, stop.data, num)
  result = utils._maybe_tf_cast(result, dtype)
  if retstep:
...
# NOTE: np.concatenate is a builtin function, so np_doc doesn't work.
def concatenate(arrays, axis=0):
  ...
  axis = utils.canonicalize_axis(axis, arrays[0].ndim)
  dtype = utils.result_type(*arrays)
  arrays = [utils._maybe_tf_cast(array, dtype) for array in arrays]
  return utils.tensor_to_ndarray(tf.concat(arrays, axis))


# NOTE: np.tile is a builtin function, so np_doc doesn't work.
def tile(a, reps):
  ...
  (1, 1, 3) for 3-D replication. If this is not the desired behavior, promote a
  to d-dimensions manually before calling this function.

  If a.ndim > d, reps is promoted to a.ndim by pre-pending 1’s to it. Thus for
  an A of shape (2, 3, 4, 5), a reps of (2, 2) is treated as (1, 1, 2, 2).
  """
  a = array_creation.array(a)
  if isinstance(reps, int):
    reps = (reps,)
  a = array_methods.reshape(a, (1,) * (len(reps) - a.ndim) + a.shape)
